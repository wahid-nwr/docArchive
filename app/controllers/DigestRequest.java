...
public class DigestRequest {
	...
	public boolean isAuthorized() {
		UserModel user = UserModel.find("byName", params.get("username")).first();
		if (user == null) {
			throw new UnauthorizedDigest(params.get("realm"));
		}
		System.out.println("putting user to session::"+params.get("username"));
		Session.current().put("apiUser", params.get("username"));
